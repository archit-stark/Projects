...
# def simulate_sadf_distribution_gpu(B, T, num_series, coint_frac, num_of_rw, multi):
#    B           = int(B)
#    T           = int(T)
#    num_series  = int(num_series)
#    num_of_rw   = int(num_of_rw)

#    # 2) Compute indices
#    split_point = int(T * coint_frac)
#    min_win     = int((0.01 + 1.8 / np.sqrt(T)) * T)

#    # 3) Cast multipliers to GPU
#    multipliers_gpu = cp.asarray(multi, dtype=cp.float64)  # shape (num_series,) or scalar

#    # 4) Build the cointegrated matrix on GPU
#    e_gpu      = cp.random.standard_normal((T, num_series))
#    common_rw  = cp.cumsum(cp.random.standard_normal(T)).reshape(-1, 1)
#    mat_gpu    = common_rw * multipliers_gpu + e_gpu

#    # 5) Pre-break OLS entirely on GPU via lstsq
#    Y_gpu          = mat_gpu[:split_point, 0]                   # (split_point,)
#    X_gpu          = mat_gpu[:split_point, 1:]                  # (split_point, num_series-1)
#    intercept_col  = cp.ones((split_point, 1), dtype=cp.float64)
#    Xc_gpu         = cp.concatenate([intercept_col, X_gpu], 1)  # (split_point, num_series)
#    params_gpu, *_ = cp.linalg.lstsq(Xc_gpu, Y_gpu, rcond=None) # (num_series,)
#    a              = float(params_gpu[0].get())                 # intercept → Python float
#    b_gpu          = params_gpu[1:]                             # slopes on GPU

#    # 6) Pre-break residuals on GPU
#    pre_resid_gpu = Y_gpu - (a + X_gpu @ b_gpu)

#    # 7) Allocate GPU array for stats
#    sadf_stats_gpu = cp.empty(B, dtype=cp.float64)

#    # 8) Main simulation loop (all GPU except the final sadf call)
#    for i in range(B):
#        # simulate the post-break random walks
#        shape_rw = (T - split_point, num_of_rw * num_series)
#        innov_gpu = cp.random.standard_normal(shape_rw)
#        last_vals = mat_gpu[split_point - 1, : shape_rw[1]]
#        rw_gpu    = cp.cumsum(innov_gpu, axis=0) + last_vals
#        mat_gpu[split_point :, : shape_rw[1]] = rw_gpu

#        # compute residuals
#        Y1_gpu       = mat_gpu[split_point:, 0]
#        X1_gpu       = mat_gpu[split_point:, 1:]
#        post_resid   = Y1_gpu - (a + X1_gpu @ b_gpu)
#        full_resid   = cp.concatenate([pre_resid_gpu, post_resid])

#        # pull back to CPU for your sadf()
#        resid_cpu    = cp.asnumpy(full_resid)
#        sadf_stats_gpu[i] = sadf(resid_cpu, min_win)
#        loading_bar(i + 1, B)

#    # 9) Bring stats home & compute critical values
#    sadf_stats = cp.asnumpy(sadf_stats_gpu)
#    crit90 = np.percentile(sadf_stats, 90)
#    crit95 = np.percentile(sadf_stats, 95)
#    crit99 = np.percentile(sadf_stats, 99)

#    # 10) (Optional) Plot
#    plt.figure(figsize=(6,4))
#    plt.hist(sadf_stats, bins=40, density=True, alpha=0.7, color='steelblue')
#    plt.axvline(crit90, color='orange', linestyle='--', label='90%')
#    plt.axvline(crit95, color='red',    linestyle='--', label='95%')
#    plt.axvline(crit99, color='purple', linestyle='--', label='99%')
#    plt.title(f"SADF Null Distribution (T={T})")
#    plt.xlabel("SADF Statistic")
#    plt.ylabel("Density")
#    plt.legend()
...
#    # 11) Return
#    return pd.DataFrame(
#        {"Critical Value": [crit90, crit95, crit99]},
#        index=["90%", "95%", "99%"]
#    )
...
def simulate_sadf_distribution_gpu(B, T, num_series, coint_frac, num_of_rw, multi):
    ...
    common_fac  = random_walk(T).reshape(-1, 1)
